...
// Храним настройки локально для UI
...
function renderMenu(state) {
    ...
    switch(state) {
        ...
        case STATES.SETTINGS:
            menuTitle.innerText = 'Конфигурация';
            backBtn.onclick = () => renderMenu(STATES.MAIN);
            renderSettingsUI();
            break;
    }
...
function renderSettingsUI() {
    ...
    charBox.className = 'bg-gray-800/50 rounded-2xl p-4 border border-gray-700';
    charBox.innerHTML = `<div class="text-[10px] font-bold text-geminiAccent uppercase tracking-widest mb-4">Настройки характера</div>`;
    ...
    sliders.forEach(s => {
        ...
        row.innerHTML = `
            <div class="flex justify-between text-[12px] mb-1">
                <span class="text-gray-400 flex items-center gap-1"><span class="material-icons-outlined text-sm">${s.icon}</span> ${s.label}</span>
            ...
            <input type="range" min="0" max="10" value="${userSettings[s.id]}" 
                ...
                oninput="updateSetting('${s.id}', this.value)">
        `;
    ...
    contextBox.innerHTML = `
        <div class="text-[10px] font-bold text-geminiAccent uppercase tracking-widest mb-4">Контекстные сообщения</div>
        <div class="flex justify-between text-[12px] mb-1">
            <span class="text-gray-400">Глубина памяти (сообщений)</span>
        ...
        <input type="range" min="1" max="50" value="${userSettings.contextLimit}" 
            class="w-full accent-geminiAccent h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer"
            oninput="updateSetting('contextLimit', this.value)">
        <p class="text-[9px] text-gray-600 mt-2">Определяет, сколько последних сообщений бот учитывает при ответе.</p>
    `;
    container.appendChild(contextBox);

    // Кнопка сохранения
    const saveBtn = document.createElement('button');
    saveBtn.className = 'w-full p-4 bg-geminiAccent text-black font-bold rounded-2xl active:scale-95 transition-all shadow-lg text-sm';
    saveBtn.innerText = 'СОХРАНИТЬ И ПРИМЕНИТЬ';
...
async function saveSettingsToServer() {
    ...
    setTimeout(() => { btn.innerText = 'СОХРАНИТЬ И ПРИМЕНИТЬ'; }, 1500);
}

// Стандартные функции рендера (те же, что в прошлый раз)
...
async function syncDialogs() {
    ...
    if (data.list) {
        data.list.forEach(d => {
            ...
            item.innerHTML = `<div class="flex-1 truncate" onclick="selectChat('${d.id}', '${d.name}')"><div class="text-sm font-bold text-white truncate">${d.name}</div></div>
...
function openRenameChat() {
    showModal('Имя чата', `<input id="modalInput" type="text" class="w-full bg-gray-800 border-gray-700 rounded-xl p-3 text-white" value="${chatNameDisplay.innerText}">`, async () => {
        ...
        chatNameDisplay.innerText = name; modalOverlay.classList.add('hidden'); syncDialogs();
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    loadUserSettings(); // Грузим ползунки при старте
